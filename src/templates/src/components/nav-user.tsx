...
export function NavUser() {
  ...
  const { data: appList, isLoading: isAppListLoading } = useQuery<
    AppList[],
    Error
  >({
  ...
  if (!isLoading && !isAppListLoading) {
    if (appList && appList?.length) {
      const isAccessible = appList.filter((app) => app.name === "Sales");
        ...
        <DropdownMenu>
          ...
          >
            <div className="p-2 overflow-auto max-h-96 min-h-20">
              <div className="grid justify-center grid-cols-3 rounded-lg cursor-pointer gap-x-4 gap-y-4">
                {isLoading ? (
                  ...
                  appList?.map((el, i) => {
                    return (
                      <Link
                        to={`${el.url}?tkn=${localStorage.getItem(
                          "refreshToken"
                        )}`}
                        target="_blank"
                        key={i}
                        className="grid justify-center px-2 rounded-xl hover:bg-accent hover:text-accent-foreground"
                      >
                        <div className="w-20 h-16 p-2 pb-0 rounded-xl">
                          <img
                            width={100}
                        ...
                        <div className="py-2 text-xs font-bold text-center min-w-23">
                          {el.name}
                        </div>
                      </Link>
                    );
        ...
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuItem>
              <Link
                to={"/redirect"}
                className="flex items-center justify-center w-full gap-2 px-2 py-1"
              >
                <LogOut size={18} />
                Logout
              </Link>
