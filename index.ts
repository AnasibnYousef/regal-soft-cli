...
const logTaskCompletion = (taskName: string, duration: number) => {
    console.log(
        chalk.gray(`├─ ${taskName}: ${(duration / 1000).toFixed(2)}s`)
    );
};
...
const validateModuleName = (input: string): string | true => {
    ...
    if (!/^[A-Z]/.test(input)) return "Module name must start with a capital letter";
...
async function createProjectDirectory(projectPath: string): Promise<void> {
    ...
    try {
        ...
        spinner.error({ text: chalk.red(`Failed to create project directory: ${(error as Error).message}`) });
        throw error;
    }
}
...
async function createEnvFile(projectPath: string, config: ProjectConfig): Promise<void> {
...
async function copyTemplates(projectPath: string, config: ProjectConfig): Promise<void> {
