...
async function createProjectDirectory(projectPath) {
    ...
    try {
        ...
        spinner.success({
            text: chalk.green(`✓ Project directory created (${(duration / 1000).toFixed(2)}s)`),
        });
    }
    catch (error) {
        spinner.error({
            text: chalk.red(`Failed to create project directory: ${error.message}`),
        });
        throw error;
    }
}
async function initializeGit(projectPath) {
    ...
    try {
        ...
        const duration = timer.end();
        spinner.success({
            text: chalk.green(`✓ Git repository initialized (${(duration / 1000).toFixed(2)}s)`),
        });
    }
    catch (error) {
...
async function createViteApp(projectPath) {
    ...
    try {
        ...
        const duration = timer.end();
        spinner.success({
            text: chalk.green(`✓ Vite application created (${(duration / 1000).toFixed(2)}s)`),
        });
    }
    catch (error) {
...
async function createDirectoryStructure(projectPath) {
    ...
    try {
        ...
        const duration = timer.end();
        spinner.success({
            text: chalk.green(`✓ Project structure created (${(duration / 1000).toFixed(2)}s)`),
        });
    }
    catch (error) {
...
VITE_PUBLIC_API_URL=${config.apiUrl}`;
        ...
        const duration = timer.end();
        spinner.success({
            text: chalk.green(`✓ Environment file created (${(duration / 1000).toFixed(2)}s)`),
        });
    }
    catch (error) {
...
async function main() {
    let projectPath = null; // Track the created folder
    try {
        ...
        console.log(chalk.gray("1. Ensure you have Node.js 20 or higher installed"));
